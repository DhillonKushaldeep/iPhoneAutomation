 ...
 *
 */
public class TC_103_Sorting_ByType extends BaseTest{

	@Test
	public void Sorting_ByType() throws InterruptedException {
		...
		List<String> history = helpAndSettingsLink.captureTimeLog();		//TIME
		List<String> beforeSorted = history;  
		Thread.sleep(2000); 	
	
		Collections.sort(history,  Collections.reverseOrder());
		Assert.assertEquals(history, beforeSorted);
		...
		List<String> history1 = helpAndSettingsLink.captureLanguageLog();	//LANGUAGE
		List<String> beforeSorted1 = history1;  
		Thread.sleep(2000); 	
	
		Collections.sort(history1,  Collections.reverseOrder());
		Assert.assertEquals(history1, beforeSorted1);
		System.out.println(history1);
		System.out.println(beforeSorted1);
