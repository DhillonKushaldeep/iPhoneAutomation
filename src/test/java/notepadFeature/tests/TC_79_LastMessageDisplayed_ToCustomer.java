 ...
 * 
 * The Notepad should display only the latest message sent by the interpreter.
 * 
 */
public class TC_79_LastMessageDisplayed_ToCustomer extends BaseTest{

	@Test
	public void INT_SendMessageDuringCall() throws InterruptedException {
	 ...
	 * 
	 * Send First message : This is Test
	 * Send Second message: This is Test again
	 */
		
		Notepad notepad = new Notepad(driver);
		notepad.verifyMessageDisplayedInNotepad();
		System.out.println("\"This is a TEST\" is displayed in TEXTBOX");
		
        MobileElement element = (MobileElement) driver.findElementByXPath("	//XCUIElementTypeTextView[@name=\\\"whiteboard\\\"]");
		
		String message = element.getText();
		String SucessMessage = "This is first message\n "
				             + "This is test again";	                   
						 
	    assertEquals(message, SucessMessage);					
		Thread.sleep(5000);	
		
		//Later on, when we integrate LINC, then we will first check the LINC messages in NOTEPAD and then we will comaore them with Insight Notepad messages.
